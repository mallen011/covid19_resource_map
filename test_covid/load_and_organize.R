...
####Poverty
poverty <- read.csv("csv/US_counties_poverty.csv")
...
counties[["NAME"]] <- 
  with(counties, ifelse(counties$STATE_NAME == 'MA', paste0(counties$NAME, "-", counties$STATE_NAME), counties$NAME))
...
####Industry
library(stringr)
industry <- read.csv("csv/industry.csv")
...
delete2 <- c("Unassigned", "Out of", "Bear River")
...
df$Province_State <- gsub("Rhode Island", "RI", df$Province_State)
df[["Admin2"]] <- 
  with(df, ifelse(Province_State == 'RI', paste0(Admin2, "-", Province_State), Admin2))
...
df$Province_State <- gsub("West VA", "WV", df$Province_State)
df[["Admin2"]] <- 
...
death$Province_State <- gsub("Rhode Island", "RI", death$Province_State)
death[["Admin2"]] <- 
  with(death, ifelse(Province_State == 'RI', paste0(Admin2, "-", Province_State), Admin2))
...
death$Province_State <- gsub("West VA", "WV", death$Province_State)
death[["Admin2"]] <- 
...
df$death <- death$value[match(df$date, death$date)]
